...
- Agent-native cloud control plane 是另一種 runtime surface：coding agent 不必託管在平台內，也能透過 machine-readable identity / project state、CLI actions、skills、service APIs 與 docs，直接操作 database、auth、storage policy、functions、compute、model gateway 與 deployment。這類 surface 的最低契約不能只有 action reach；還應讓 agent 在 mutation 前後檢查 active identity、project、environment、configuration 與 resulting state。
...
- [InsForge Machine-Readable Overview](../articles/2026-07-02-insforge-machine-readable-overview.md): supplies a concrete agent-facing cloud control plane where CLI, skills, machine-readable state, service APIs, and agent-oriented documentation extend the coding-agent loop into infrastructure provisioning and deployment.
...
- agent-native cloud CLI 應如何暴露 plan / diff / dry-run、identity、environment、least privilege、idempotency、approval、audit 與 rollback，才能讓 agent operability 同時成為 agent-safe operability
