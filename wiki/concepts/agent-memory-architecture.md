...
- static corpus 的 vector RAG 也需要 knowledge-unit governance；question-answer packet / IdeaBlock 這類單位能把 idea boundary、source、version state 與 access metadata 綁在同一個 retrieval object 上，避免把文件 chunk 當成唯一知識單位
...
- Agentic technical-debt framing adds a practical split: short-term scratchpad, session memory, long-term memory, semantic facts, episodic events, and procedural skills should not be collapsed into one vector store, because each layer has different drift, privacy, retrieval, and deletion risks.
...
- Static document corpus 應在何時使用 raw chunks、question-answer packets、layout-aware page units 或 graph triples，才能在 retrieval accuracy、source faithfulness、governance 與 ingest cost 之間取得平衡？
