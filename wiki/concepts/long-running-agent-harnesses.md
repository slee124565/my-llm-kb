...
- 大型程式遷移顯示長任務的 control plane 可以由 rulebook、dependency map、gap inventory、可從磁碟重建的 queue 與單一 build daemon 組成：agent 平行產出 patch，但共享且昂貴的 rebuild 被批次序列化，compiler / test / behavior diff 則持續把下一個 fixer 任務餵回 loop。
- 若 reviewer 在多個 batch 重複發現同一類錯誤，harness 的正確修復是更新 rulebook、重生受影響批次並重跑，而非逐檔覆寫 agent 輸出；這讓流程而非一次性 code patch 成為可演化的單位。
